...
- **Stencil unexercised in-repo.** `gen/nodeve-projected.yaml` is the downstream pre-database contract (closed per-feature-type quantity sets, `node_type` pins), but no check here validates rows against it — a projection bug surfaces downstream first. The deleted `validate` script targeted the pre-nesting single-file shape; its replacement is a row-shaped smoke check: known-good catalog rows pass their projected class, a wrong-quantity row fails.
