 ...
 *
 * Flags ONLY when the output provably mirrors the input — zero false positives
 * on legitimate renames (`c => ({ value: c.id })`) or additions (`x => ({...x, n})`):
 *
 ...
 *
 * Routing a hit is a JUDGEMENT call: delete the reshape and pass the value as-is,
 * use a pick/clone helper if narrowing is genuinely the point, or allowlist a
 * confirmed boundary (config `reshape.allowlist`, `relPath::kind::keys`). A rename
 * that only dodges the match keeps the smell.
 *
 * BLOCKS by default: any non-allowlisted finding exits 1. `--warn` downgrades to
 * report-only (exit 0). Pass explicit paths (lefthook `{staged_files}`) to scope.
 */
...
// Silent on success — advisory checks shouldn't log on every clean commit.
if (findings.length === 0) {
	if (verbose) console.log('reshape: clean');
	process.exit(0);
}

findings.sort((a, b) => a.rel.localeCompare(b.rel) || a.line - b.line);
console.log(
	`reshape: ${findings.length} reshape(s) reproduce the input shape (no-op / pick / clone).\n` +
		`Pass the value as-is, or use a pick/clone helper if narrowing is the point — then allowlist:\n`,
);
const pad = Math.max(...findings.map((f) => f.kind.length));
for (const f of findings) console.log(`  ${f.kind.padEnd(pad)}  { ${f.keys} }  ${f.rel}:${f.line}`);
