...
- `concept_settings.compose` overlays a **same-layer** sibling's whole def — an archetype composes archetypes, a feature composes features; it NEVER reaches down a layer. A feature reaches an archetype via the `feature:` map, not `compose` (enforced in `kit/compile.ts` `resolveSiblingBySlug`).
