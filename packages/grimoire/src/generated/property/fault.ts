// GENERATED by `pnpm generate` from concepts/property/ via kit/compile.ts +
// kit/emit-types.ts. Do not edit by hand — edit the YAML and regenerate. The snake_case
// TypeBox schema transcribed from the compiled draft-07 projection (its own <name>.schema.json);
// `Value.Check(<Name>Schema, …)` validates, the camelCase type is the surface consumers code
// against. The `export default` is the full DATA tree (labels/refs/ui) — the .ts twin of the
// sibling .json; composed slots spread their sibling `<Name>Data` default so a shape lives once.

import { type TSchema, Type } from '@sinclair/typebox';

export const FaultSchema: TSchema = Type.String({"minLength":1});

export type Fault = string;

export type FaultDataT = { readonly "description": { readonly "en": "Slug naming the fault-set slug group a categorical decode publishes into (fault_code). Exclusive with `state` (a run-state slug group) — an entry is one or the other." }; readonly "schema": { readonly "minLength": 1; readonly "type": "string" }; readonly "title": { readonly "en": "Fault" } };

const _data: FaultDataT = {
	"description": {
		"en": "Slug naming the fault-set slug group a categorical decode publishes into (fault_code). Exclusive with `state` (a run-state slug group) — an entry is one or the other."
	},
	"schema": {
		"minLength": 1,
		"type": "string"
	},
	"title": {
		"en": "Fault"
	}
};
export default _data;
