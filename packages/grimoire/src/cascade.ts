...
const isLeaf = (f: string): boolean =>
	f.endsWith('.yaml') && !isDefaults(f) && !f.endsWith('.example.yaml');
	...
	for (const entry of names.sort((a, b) => a.name.localeCompare(b.name))) {
		if (entry.isDirectory()) {
			...
			if (typeof archetype !== 'string') {
				throw new Error(
					`grimoire catalog leaf ${join(dir, entry.name)} has no \`archetype_id\` (declare it in a _defaults.yaml)`,
				);
			}
...
export function loadCascade(root: string): CascadeEntry[] {
	...
	walkCascade({ dir: root, segments: [], inherited: {}, out });
