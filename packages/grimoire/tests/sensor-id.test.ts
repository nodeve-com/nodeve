...
const featureSlug = (feature: string): string => {
	...
	const id =
		node && typeof node === 'object'
			? (node as { identity?: { slug?: unknown } }).identity
			: undefined;
...
describe('sensorId over the dtsu666 register map', () => {
	...
	test('intervalSensorId composes a baked sensor slug with an interval slug', () => {
		expect(intervalSensorId('grid_meter_live_ac_active_power', 'grid_neutral')).toBe(
			'grid_meter_live_ac_active_power_grid_neutral',
		);
		expect(() => intervalSensorId('grid_meter_live_ac_active_power', 'Grid Neutral')).toThrow(
			/not a slug/,
		);
	});
});
