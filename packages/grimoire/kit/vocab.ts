...
// concepts/enumeration/<name>/*.yaml; `bun run grimoire-generate` bakes each enumeration into
...
import { quantityKinds as quantityKindsGenerated } from '../generated/enumeration/quantity_kind.ts';
...
export type QuantityKind = Term;
/** `code → QuantityKind`, baked from concepts/enumeration/quantity_kind/*.yaml. */
export const quantityKinds: Record<string, QuantityKind> = quantityKindsGenerated;

/** The quantity kind a `code` names, or `undefined` if no such kind is registered. */
export const quantityKind = (code: string): Term | undefined => quantityKinds[code];

/** Resolve a kind's crosswalk to an external `registry` → that registry's term (mirrors
 *  `Vocab.crosswalk`; a kind may crosswalk to some registries and not others). */
export const quantityKindCrosswalk = (code: string, registry: string): string | undefined =>
	quantityKinds[code]?.refs?.find((r) => r.registryId === registry)?.term;
