...
- **`interval_kind?`** — the closed TOP classifier (`enumeration/interval_kind`). Three are STATELESS (`f(reading) → in-band?`): `measurable` (instrument-readable span), `rating` (a rated capability tier), `zone` (a named operating region → a boolean "in this region" sensor). One is STATEFUL (`f(reading, prior_state) → on/off`): `threshold` (a hysteretic trigger). DERIVED, never authored on the base axes: `threshold` from a `direction`, `rating` from a rating tier, `zone` from a zone name; `measurable` authored directly.
- **`rating?`** — the tier of a `rating` band (`enumeration/rating`): `continuous` / `intermittent` / `short_term` / `startup` / `shutdown` / `survival` / `protection_required` / `nominal` (a bounds-free nameplate value). The capability ENVELOPE — how far / how long it's rated to be pushed.
...
- **`bound?` / `direction?` / `hysteresis?`** — the STATEFUL trigger fields (`interval_kind: threshold`, folding in the former standalone `threshold` feature). `bound` = the trip value; `direction` (`above`/`below`) = the activating side — the stateful bit `min`/`max` position can't encode; `hysteresis` = the release deadband past `bound`. A PV inverter's startup is `{ zone: running, bound: 140, direction: above, hysteresis: 50 }` — trips ON above 140 V, releases (dropout) below 90 V; a heating thermostat is `{ bound: 20, direction: below, hysteresis: 1 }`. The `zone` name (or `identity.slug`) handles the boolean sensor.
