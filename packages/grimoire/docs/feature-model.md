...
- **`interval_kind?`** — the closed TOP classifier (`enumeration/interval_kind`). Three are STATELESS (`f(reading) → in-band?`): `measurable` (instrument-readable span), `rating` (a rated capability tier), `zone` (a named operating region → a boolean "in this region" sensor). One is STATEFUL (`f(reading, prior_state) → on/off`): `threshold` (a hysteretic trigger). **Always authorable explicitly**; omit it and it's DERIVED from the base axes — `threshold` from a `trigger_on`, `rating` from a rating tier, `zone` from a zone name. `measurable` isn't derivable from bounds, so a bare span must author it.
...
- **`min?`/`max?` + `trigger_on?`** — the STATEFUL trigger fields ([`interval_kind: threshold`](threshold.md), folding in the former standalone `threshold` feature). `min`/`max` are the two hysteresis edges (the deadband/hold zone — the same `min`/`max` a stateless band uses); `trigger_on` (`above`/`below`) picks which side is ON, the region OUTSIDE the deadband — the one irreducible bit `min`/`max` can't encode, and its presence is what makes the row stateful. A PV inverter's startup is `{ zone: running, trigger_on: above, min: 90, max: 140 }` — trips ON above 140 V, releases (dropout) below 90 V; a heat-mode thermostat is `{ trigger_on: below, min: 20, max: 21 }`. `value?` may ride along as an optional nominal setpoint (`severity: nominal`), display context only. The `zone` name (or `identity.slug`) handles the boolean sensor.
...
plus `value?` (the one role-neutral point — a zone operating value like Vmp/Voc, the threshold trip, or a rated centre graded `severity: nominal`), one-sided `min?`/`max?`, band-around-value (absolute `tolerance?`/`tolerance_lower?`/`tolerance_upper?`, relative `margin?`/`margin_lower?`/`margin_upper?`, or the `fraction_lower?`/`fraction_upper?` multiplier sugar), `unit?`, `conditions?`.
