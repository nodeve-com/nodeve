...
const PUBLISHED_BY: Record<string, string> = {
	...
	brick: 'brickschema',
	cim: 'iec',
	ha_device_class: 'home_assistant',
	ha_state_class: 'home_assistant',
	iana_iftype: 'iana',
	iana_protocol: 'iana',
	iana_uri_scheme: 'iana',
	iec_60381: 'iec',
	iec_60445: 'iec',
	iec_61850: 'iec',
	iso_11898: 'iso',
	iso_80000: 'iso',
	jedec: 'jedec',
	modbus_org: 'modbus_organization',
	prometheus: 'cncf',
	qudt_quantity_kind: 'qudt',
	saref4grid: 'etsi',
	seas: 'w3c',
	skos: 'w3c',
	sosa: 'w3c',
...
const NEW_ORG_BY_SLUG: Record<string, { title: string; url: string }> = {
	...
	bipm: {
		title: 'Bureau International des Poids et Mesures (BIPM/JCGM)',
...
function registryEntry(options: {
	...
	const { slug, comment, title, url, iriTemplate } = options;
	const pub = PUBLISHED_BY[slug];
	if (!pub) throw new Error(`no published_by for registry ${slug}`);
	const titleBlock =
		typeof title === 'object' && title
			? Object.entries(title as Record<string, string>)
					.map(([k, v]) => `  ${k}: ${yamlStr(v)}`)
					.join('\n')
...
if (existsSync(ENUM_REGISTRY)) {
	...
	writeFileSync(
		join(CATALOG_REGISTRIES, '_defaults.yaml'),
	...
	for (const f of readdirSync(ENUM_REGISTRY)) {
		...
		writeFileSync(
			join(CATALOG_REGISTRIES, `${slug}.yaml`),
			registryEntry({
				slug,
				comment,
				title: doc.title,
				url: identity.url,
				iriTemplate: identity.iri_template,
			}),
		);
	}
...
writeFileSync(
	join(CATALOG_REGISTRIES, 'posix.yaml'),
	registryEntry({
		slug: 'posix',
		comment:
		...
		title: { en: 'POSIX (IEEE Std 1003.1)', pt: 'POSIX (IEEE Std 1003.1)' },
